...
async def staff_dashboard(current_user: dict = Depends(require_staff)):
    ...
    # Upcoming sessions for receptionist; recent scores for clinical_assistant
    extra = {}
...
async def get_patient_detail(patient_id: str, current_user: dict = Depends(require_staff)):
    admin = get_supabase_admin()
    role = current_user["role"]
    ...
    profile = admin.table("profiles").select("*").eq("id", patient_id).single().execute().data or {}

    # Sessions visible to everyone
    ...
    # Clinical assistants get full detail (permissions + scores)
...
async def allocate_patient_to_doctor(
    ...
    doctor = admin.table("doctors").select("id").eq("id", body.doctor_id).single().execute().data
    if not doctor:
