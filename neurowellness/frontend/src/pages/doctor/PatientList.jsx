...
const S = {
  ...
  avatar: { width: '36px', height: '36px', borderRadius: '50%', background: '#4f46e5', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', fontSize: '14px', marginRight: '10px' },
}
...
export default function PatientList() {
  ...
  return (
    <DoctorLayout>
      <div style={S.header}>
        <h1 style={S.h1}>My Patients ({patients.length})</h1>
        <input
          style={S.search}
      ...
      {loading ? <LoadingSpinner /> : error ? <p style={{ color: 'red' }}>{error}</p> : (
        <div style={S.card}>
          <table style={S.table}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} style={{ ...S.td, textAlign: 'center', color: '#9ca3af' }}>No patients found</td></tr>
              ) : filtered.map(p => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td style={S.td}>{[prof.city, prof.state].filter(Boolean).join(', ') || '—'}</td>
                    <td style={S.td}>{p.created_at ? new Date(p.created_at).toLocaleDateString() : '—'}</td>
                    <td style={S.td}>
                      <Link to={`/doctor/patients/${p.id}`} style={S.viewBtn}>View Details</Link>
                    </td>
                  </tr>
                )
              })}
