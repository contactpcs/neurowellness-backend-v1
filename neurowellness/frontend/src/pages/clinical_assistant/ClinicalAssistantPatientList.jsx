...
const S = {
  ...
  avatar: { width: '36px', height: '36px', borderRadius: '50%', background: '#7c3aed', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', fontSize: '14px', marginRight: '10px' },
}

export default function ClinicalAssistantPatientList() {
              ...
              {!filtered.length ? (
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td style={S.td}>
                      <Link to={`/clinical-assistant/patients/${p.id}`} style={S.viewBtn}>View Details</Link>
                    </td>
                  </tr>
                )
              })}
